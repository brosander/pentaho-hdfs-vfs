...
public class MapRFileNameParser extends URLFileNameParser {
  ...
  public FileName parseUri( VfsComponentContext context, FileName base, String filename ) throws FileSystemException {
    URLFileName result = (URLFileName) super.parseUri( context, base, filename );
    if ( result.getQueryString() == null && base instanceof URLFileName ) {
      result =
          new URLFileName( result.getScheme(), result.getHostName(), result.getDefaultPort(), result.getPort(), result
              .getUserName(), result.getPassword(), result.getPath(), result.getType(), ( (URLFileName) base )
              .getQueryString() );
    }
    return result;
  }
